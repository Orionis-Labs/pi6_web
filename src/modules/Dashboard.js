...
// const price = WalletItems.map((item) => (item.price))
// const b = graph.push(parseInt(WalletItems.map((item) => item.amount.filter((am) => (item.denom === `${am.denom}`)).map((ite) => ite.amount)) * price));
// graph.push(b);
const DashboardModule = ({ WalletItems }) => {
  const graph = [];
  for (const obj of WalletItems) {
    graph.push(obj.amount.filter((ite)=>(obj.denom === `${ite.denom}`)).map((it)=>it.amount))
  }
      ...
      <section className="zl_dashboard_page">
        ...
        <div className="zl_all_page_comman_content">
          <div className="zl_chart_box_heading_date">
            ...
            <div id="chart">
               <ReactApexChart options={option.options} series={option.series} type="donut" height='300' />
            </div>
          <div className="zl_all_page_comman_total_price">
            <p className="zl_all_page_total_price_heading">Total Revenue</p>
            <h2 className="zl_all_page_total_price_text">eh</h2>
